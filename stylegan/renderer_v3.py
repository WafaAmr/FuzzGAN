...
class Renderer:
    ...
    def _render_impl(self, res,
        ...
        print(f"class: {class_idx}, w0_seed: {w0_seeds[0][0]}, mixclass_idx:{mixclass_idx} , stylemix_seed:{stylemix_seed}, stylemix_idx:{stylemix_idx}, patch_idxs:{patch_idxs}")
        ...
        if patch_idxs is not None:
            res["patch_mix"] = True
            res["patch_idxs"] = dnnlib.EasyDict()

            for elm, stylemix_elm in enumerate(patch_idxs):
                res.patch_idxs[f'stylemix_{elm}'] = stylemix_elm

                res[f'stylemix_{elm}'] = dnnlib.EasyDict()
                elm_res = res[f'stylemix_{elm}']

                elm_res.stylemix_elm = stylemix_elm
                elm_res.w = w.detach().clone()

                elm_w = elm_res.w
                stylemix_elm = [idx for idx in stylemix_elm if 0 <= idx < G.num_ws]

                if stylemix_seed is not None and len(stylemix_elm) > 0:
                    elm_w[:, stylemix_elm] = all_ws[stylemix_seed][np.newaxis, stylemix_elm]
                elm_w += w_avg

                elm_res.update(self._render_impl_run(dnnlib.EasyDict(), elm_w))
        else:
            ...
            res = self._render_impl_run(res, w)

    def _render_impl_run(self, res, w):
        G = self.G
        random_seed = self.params.random_seed
        noise_mode = self.params.noise_mode
        force_fp32 = self.params.force_fp32
        layer_name = self.params.layer_name
        sel_channels = self.params.sel_channels
        base_channel = self.params.base_channel
        img_scale_db = self.params.img_scale_db
        img_normalize = self.params.img_normalize
        to_pil = self.params.to_pil
        untransform = self.params.untransform
