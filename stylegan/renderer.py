...
from stylegan import legacy # pylint: disable=import-error
...
class Renderer:
    ...
    def get_network(self, pkl, key, **tweak_kwargs):
        ...
        if net is None:
            try:
                if 'stylegan2' in pkl:
                    from stylegan.networks_stylegan2 import Generator
                elif 'stylegan3' in pkl:
                    from stylegan.networks_stylegan3 import Generator
                else:
    ...
    def _render_drag_impl(self, res,
        ...
        **kwargs
        ...
        # # print(trunc_psi)
        # print(G.c_dim)
        # print(f'Label tensor: {label.tolist()}')
        # print("###############################################")
        ...
        if is_drag:
            ...
            feature_idx = min(feature_idx, len(feat)-1)
            # feature_idx = 14
            # print("------------------------------------")
            # print(len(feat))
            # print(feature_idx)
            ...
            with torch.no_grad():
                for j, point in enumerate(points):
                    ...
                    # print(f'r: {r}, up: {up}, down: {down}, left: {left}, right: {right}')
            ...
            for j, point in enumerate(points):
                ...
                # print(f'direction: {[targets[j][1] - point[1], targets[j][0] - point[0]]}')
                ...
                if torch.linalg.norm(direction) > 1:
                    ...
                    if r == 0:
                        r = 1
                    relis, reljs = torch.where(distance < r)
