...
# ==============================================================================
    ...
    def on_change_update_class_idx(class_idx, global_state):
        """Function to handle generation seed change.
        1. Set seed to global_state
        2. Re-init images and clear all states
        """

        global_state["params"]["class_idx"] = int(class_idx)
        init_images(global_state)
        clear_state(global_state)

        return global_state, global_state['images']['image_show']

    form_class_idx.change(
        on_change_update_class_idx,
        inputs=[form_class_idx, global_state],
        outputs=[global_state, form_image],
    )
        ...
        if len(global_state["points"]) == 0:
            ...
            while True:
                ...
                renderer._render_drag_impl(
                    ...
                    # img_normalize   = False,
                    # untransform     = False,
                    class_idx=global_state['params']['class_idx'],
                    is_drag=True,
                    to_pil=True)
...
app.launch(share=False, server_name="0.0.0.0" if args.listen else "127.0.0.1")
# 4294967295