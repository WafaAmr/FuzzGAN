...
class mimicry:
    ...
    def search(self):
        root = f"mnist/eval/final/HQ-100/{self.class_idx}/"
        save_BF = False

        frontier_seeds = set()
        # con_class = {}
        while len(frontier_seeds) < self.search_limit:
            state = self.state
                    ...
                    for stylemix_cls in self.con_class[self.class_idx]:
                        ...
                        while not found_mutation and not tried_all_layers and self.stylemix_seed < self.stylemix_seed_limit:
                            ...
                            for idx, layer in enumerate(self.layers):
                                ...
                                if not m_accepted:
                                    # if not m_class[0] in con_class:
                                    #     con_class[m_class[0]] = 1
                                    # else:
                                    #     con_class[m_class[0]] += 1
                                    ...
                                    if valid_mutation:
                                        ...
                                        print(f"Found Mutation: {len(frontier_seeds)}")
                                        found_mutation = True
